...
FReader.Attributes.GMOD_MODULE = "\x00gmod13_open\x00";
FReader.Options.UPX_COMPRESSED = "\x00UPX!"
 
function FReader.getosfile(fullpath)
	local tmpstr = file.Read(fullpath, "BASE_PATH")
	
	for k, v in pairs(FReader.Headers) do
		if string.StartWith(tmpstr, v) then
			return k
		end
	end
	tmpstr = nil;
